...
def run_exp(config_path):
    args = parse_args()
    # conditional import to avoid requiring private Neptune credentials when not logging
    neptune_logger = None
    if args.log:
        try:
            ...
            print(f"Neptune logger unavailable ({e}). Proceeding without external logging.")
